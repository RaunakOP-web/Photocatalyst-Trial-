...
# Semiconductor physical properties database
SEMICONDUCTOR_PROPS = {
    ...
    "in2s3":             {"bandgap_eV": 2.00, "electron_affinity_eV": 4.20,
                          "dielectric": 13.5, "crystal": 6, "density": 4.90},
    "cdse":              {"bandgap_eV": 1.74, "electron_affinity_eV": 4.95,
                          "dielectric": 10.6, "crystal": 3, "density": 5.81},
    "gan":               {"bandgap_eV": 3.40, "electron_affinity_eV": 4.10,
                          "dielectric": 9.7,  "crystal": 3, "density": 6.15},
    "unknown":           {"bandgap_eV": 3.00, "electron_affinity_eV": 4.20,
                          "dielectric": 20.0, "crystal": 1, "density": 5.00},
}

# Co-catalyst physical properties database
COCATALYST_PROPS = {
    ...
    "rgo":    {"work_function": 4.50, "d_band_center":  0.00,
               "atomic_radius": 77,  "electronegativity": 2.55, "price": 1},
    "cos2":   {"work_function": 4.80, "d_band_center": -1.30,
               "atomic_radius": 125, "electronegativity": 1.88, "price": 2},
    "ni2p":   {"work_function": 4.90, "d_band_center": -1.10,
               "atomic_radius": 124, "electronegativity": 1.91, "price": 2},
    "wc":     {"work_function": 4.95, "d_band_center": -1.60,
               "atomic_radius": 139, "electronegativity": 2.36, "price": 3},
    "fes2":   {"work_function": 4.74, "d_band_center": -0.80,
               "atomic_radius": 126, "electronegativity": 1.83, "price": 1},
    "mn":     {"work_function": 4.10, "d_band_center": -0.50,
               "atomic_radius": 127, "electronegativity": 1.55, "price": 1},
    "mos2":   {"work_function": 5.00, "d_band_center": -1.00,
               "atomic_radius": 139, "electronegativity": 2.16, "price": 2},
    "cuox":   {"work_function": 4.55, "d_band_center": -2.50,
               "atomic_radius": 128, "electronegativity": 1.90, "price": 1},
    "niox":   {"work_function": 4.85, "d_band_center": -1.50,
               "atomic_radius": 124, "electronegativity": 1.91, "price": 1},
    "ruo2":   {"work_function": 4.71, "d_band_center": -1.41,
               "atomic_radius": 134, "electronegativity": 2.20, "price": 4},
    "au-pd":  {"work_function": 5.16, "d_band_center": -2.04,
               "atomic_radius": 140, "electronegativity": 2.24, "price": 5},
    "pt-cu":  {"work_function": 5.15, "d_band_center": -2.46,
               "atomic_radius": 133, "electronegativity": 2.09, "price": 4},
...
def encode_semiconductor(name: str) -> dict:
    ...
    props = SEMICONDUCTOR_PROPS.get(name_clean, SEMICONDUCTOR_PROPS["unknown"])
...
def encode_cocatalyst(name: str) -> dict:
    ...
    props = COCATALYST_PROPS.get(name_clean, COCATALYST_PROPS["unknown"])
    return {f"cocat_{k}": v for k, v in props.items()}

def add_physical_features(df: pd.DataFrame) -> pd.DataFrame:
    semi_feats = df["host_material"].apply(lambda x: pd.Series(encode_semiconductor(x)))
    cocat_feats = df["co_catalyst"].fillna("none").apply(lambda x: pd.Series(encode_cocatalyst(x)))
    df_out = pd.concat([df, semi_feats, cocat_feats], axis=1)
    
    # Advanced Semiconductor electronic properties relative to NHE
    df_out["semi_CB_potential_NHE"] = df_out["semi_electron_affinity_eV"] - 4.5
    df_out["semi_VB_potential_NHE"] = df_out["semi_CB_potential_NHE"] + df_out["semi_bandgap_eV"]
    
    # Exciton binding energy proxy
    df_out["semi_exciton_proxy"] = 13.6 / (df_out["semi_dielectric"] ** 2 + 1.0)
    
    # Effective mass proxy
    df_out["semi_eff_mass_proxy"] = df_out["semi_density"] / (df_out["semi_dielectric"] + 1.0)
    
    # Cocatalyst Hydrogen Adsorption Free Energy (dG_H*) proxy mapping
    dg_map = {
        "pt": -0.09, "pd": -0.06, "au": 0.38, "ag": 0.40,
        "cu": 0.12, "ni": -0.15, "co": -0.25, "fe": -0.35,
        "rh": -0.12, "ir": -0.10, "ru": -0.15, "none": 0.0,
        "unknown": 0.0
    }
    df_out["cocat_dg_h_proxy"] = df_out["co_catalyst"].fillna("none").astype(str).str.strip().str.lower().map(dg_map).fillna(0.0)
    
    # Overpotential proxy
    df_out["cocat_overpotential_proxy"] = np.abs(df_out["cocat_work_function"] - 4.5)
