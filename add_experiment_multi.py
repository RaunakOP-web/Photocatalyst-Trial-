...
def parse_args():
    parser = argparse.ArgumentParser(description="Add new multi-objective experimental catalyst data and retrain GPR models.")
...
def prompt_interactive():
    print("\n--- Enter New Multi-Objective Experimental Results ---")
    data = {}
    ...
    while True:
        ...
        if fam in ["TiO2", "CdS", "g-C3N4", "ZnS", "MOF", "Other"]:
            data["family"] = fam
            break
        print("Invalid family. Choose from: TiO2, CdS, g-C3N4, ZnS, MOF, Other.")
...
def main():
    ...
    # If key fields are missing, fall back to interactive
    ...
    print(f"\nAppending new multi-objective experiment to {html_path}...")
    ...
    print("\nStep 2: Triggering Multi-Objective AutoML Model Retraining & Screening...")
    python_exe = sys.executable
    try:
        subprocess.run([python_exe, "corrected_pipeline.py"], check=True)
        print("\n=== Closed-Loop Retraining & Rescreening Completed! ===")
    except subprocess.CalledProcessError as e:
        print(f"\nError: Model retraining/screening failed with exit code {e.returncode}")
